		...
		public class FormatPreset : IDisposable
		{
			...
			public static FormatPreset[] GetPresets(string customCommand)
			{
				return new[]
				{
					...
					new FormatPreset("Matroska HEVC", "HEVC video and Vorbis audio in a Matroska container.",
						"-c:a libvorbis -c:v libx265 -f matroska", false, "mkv"),
					new FormatPreset("MP4", "AVC video and AAC audio in an MP4 container.",
						"-c:a aac -c:v libx264 -f mp4", false, "mp4"),
					new FormatPreset("MP4 HEVC", "HEVC video and AAC audio in an MP4 container.",
						"-c:a aac -c:v libx265 -f mp4", false, "mp4"),
					...
					new FormatPreset("QuickTime HEVC", "HEVC video and AAC audio in a QuickTime container.",
						"-c:a aac -c:v libx265 -f mov", false, "mov"),
